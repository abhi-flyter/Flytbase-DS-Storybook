...
- **CRITICAL: Never invent token aliases.** Do not use guessed names like `--color-fds-background`, `--color-fds-surface`, `--color-fds-text-primary`, `--color-fds-border`, `--color-fds-primary`, `--fds-color-surface`, `--fds-color-text-primary`, `--fds-color-border`, or `--fds-color-primary`; those variables are not exported by this package.
